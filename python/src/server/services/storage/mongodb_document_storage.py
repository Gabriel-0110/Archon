"""
MongoDB Document Storage Service

Handles storage of documents in MongoDB with parallel processing support.
"""
...
async def add_documents_to_mongodb(
    ...
    """
    Add documents to MongoDB with threading optimizations.
    ...
    """
    with safe_span(
        "add_documents_to_mongodb", total_documents=len(contents), batch_size=batch_size
    ) as span:
        ...
        for batch_num, i in enumerate(range(0, len(contents), batch_size), 1):
            ...
            for retry in range(max_retries):
                ...
                try:
                    await db.documents.insert_many(batch_data)
